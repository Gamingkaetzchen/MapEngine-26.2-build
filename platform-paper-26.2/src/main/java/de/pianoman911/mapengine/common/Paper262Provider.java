...
public final class Paper262Provider implements IPlatformProvider {
    ...
    public Optional<IPlatform<?>> tryProvide(Plugin plugin, IListenerBridge bridge) {
        ...
        if (IPlatformProvider.existsClass("org.bukkit.craftbukkit.CraftServer")
            ...
            return Optional.of(Paper262StaticProvider.provide(plugin, bridge));
        }
        return Optional.empty();
    }
}

final class Paper262StaticProvider {

    private Paper262StaticProvider() {
    }

    static IPlatform<?> provide(Plugin plugin, IListenerBridge bridge) {
        return new Paper262Platform(plugin, bridge);
    }
}
